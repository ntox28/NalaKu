...
const EmployeeManagement: React.FC<EmployeeManagementProps> = ({ employees, onUpdate, users }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const employeeData = { ...formData, user_id: editingEmployee?.user_id || null };
        ...
        if (editingEmployee) {
            ...
            if (error) {
                addToast(`Gagal memperbarui karyawan: ${error.message}`, 'error');
            } else {
            ...
            if (error) {
                addToast(`Gagal menambahkan karyawan: ${error.message}`, 'error');
            } else {
    ...
    const handleDelete = async (employeeId: number) => {
        if (window.confirm('Apakah Anda yakin ingin menghapus karyawan ini? Ini tidak akan menghapus akun login yang terhubung.')) {
            setIsLoading(true);
            const { error } = await supabase.from('employees').delete().eq('id', employeeId);
            
            if (error) {
                addToast(`Gagal menghapus karyawan: ${error.message}`, 'error');
            } else {
                        ...
                        <div className="overflow-y-auto pr-2 -mr-4 flex-1">
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                <div className="flex justify-end space-x-4 pt-4 flex-shrink-0">
                                    ...
                                    <button type="submit" disabled={isLoading} className="px-6 py-2 rounded-lg text-white bg-orange-600 hover:bg-orange-700 transition-colors disabled:bg-orange-300">{isLoading ? 'Menyimpan...' : (editingEmployee ? 'Simpan Perubahan' : 'Simpan')}</button>
                                </div>
                            </form>
                        </div>
                    </div>
